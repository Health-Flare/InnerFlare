...
/// inactive/paused and back to resumed on its own, whether Face ID's
/// system sheet or (more so) the separate device-credential activity
/// Android launches for a manual passcode, even though the user never
/// actually left the app. [AppLockGate]'s lifecycle observer can't
/// otherwise tell that apart from a real backgrounding, and would record
/// it as time spent away; with a short enough (or "Immediately")
/// idle-lock timeout, re-checking on the trailing "resumed" re-locks the
/// app moments after (or even before) that very authentication attempt
/// unlocks it: every tap of Unlock re-locking itself, permanently, with
/// force-closing the
...
String _$reauthenticationFlagHash() =>
    r'55cbc3cfee21873682f22a9ea864ffc788f37e91';
